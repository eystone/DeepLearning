...
void InitGraph( Graph& graph, const struct SimpleCapParams & capture ) {
    ...
    for ( int i(0); i < capture.mHeight; i++ ) {
        for ( int j(0); j < capture.mWidth; j++) {
            graph.addNode( unsigned(i * capture.mWidth + j), ConvertRGBBinToMono( unsigned(capture.mTargetBuf[i * capture.mWidth + j]) ) );
        }
    }
...
int main()
{
    ...
    for (Graph subdiveGraph : SubdividedGraphList) {
        ...
        bool haveBarcode = HaveBarcode(subdiveGraph);
    ...
    //graph.printGraph();
    graph.drawImage(unsigned(capture.mWidth));
